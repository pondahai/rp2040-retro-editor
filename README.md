...
| `Fn` + `Space` | 切換英數 / 注音 |
| `Fn` + `S` | 存檔 |
| 注音模式：注音鍵 | 組字（大千配列，一聲不用打；`3/4/6/7` 是二/三/四/輕聲） |
| 注音模式：`↑` `↓` | 移動候選字（選中的會反白） |
| 注音模式：`Enter` | 選字 |
| 注音模式：`PgUp` `PgDn` | 候選字翻頁 |
| 注音模式：`Backspace` | 退一個注音鍵 |
| 注音模式：`Esc` | 取消組字 |

> ⚠️ **選字不能用數字鍵。** 大千配列的數字鍵本身就是注音（`1`=ㄅ `2`=ㄉ
> `5`=ㄓ `8`=ㄚ `9`=ㄞ `0`=ㄢ，`3/4/6/7` 是聲調），拿來選字的話數字鍵就永遠
> 打不出注音——這是本專案在真機上被回報的第一個 bug。操作方式照
> `rp2040-retro-dict` 的 `firmware/app.c`，那一套已經在真機上用過。
